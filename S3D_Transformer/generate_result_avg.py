...
def validate(args):
    ...
    for dname in list_indata:
        ...
        for i in range(0, len(list_frames)-len_temporal, len_temporal):
            snippet = []
            for j in range(len_temporal):
                torch_img, img_size = torch_transform(os.path.join(path_indata, dname, 'images', list_frames[i+j]))

                snippet.append(torch_img)    
            clip = torch.FloatTensor(torch.stack(snippet, dim=0)).unsqueeze(0)
            clip = clip.permute((0,2,1,3,4))

            smap = process(model, clip, img_size)
            for j in range(len_temporal):
                img_save(smap, join(args.save_path, dname, list_frames[i+j]), normalize=True)


        snippet = []
        for j in range(len_temporal):
            torch_img, img_size = torch_transform(os.path.join(path_indata, dname, 'images', list_frames[len(list_frames)-len_temporal+j]))
        ...
        smap = process(model, clip, img_size)
        for j in range(len_temporal):
            img_save(smap, join(args.save_path, dname, list_frames[len(list_frames)-len_temporal+j]), normalize=True)
...
def process(model, clip, img_size):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path',default='/ssd_scratch/cvit/samyak/Results/single_frame_average', type=str)
