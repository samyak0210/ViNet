...
class SoundDatasetLoader(Dataset):
	def __init__(self, len_snippet, dataset_name='DIEM', split=1, mode='train', use_sound=False):
		''' mode: train, val, save '''
...
class DHF1KMultiSave(Dataset):
	def __init__(self, path_data, len_snippet, start_idx=-1, num_parts=4, alternate=1):
		''' mode: train, val, save '''
		self.path_data = path_data
		self.alternate = alternate
		list_indata = sorted(os.listdir(path_data))
		if start_idx!=-1:
		...
		for v in list_indata:
			for i in range(0, len(os.listdir(os.path.join(path_data,v,'images')))-self.alternate * self.len_snippet, self.alternate * self.len_snippet-1):
				self.list_num_frame.append((v, i))
			self.list_num_frame.append((v, len(os.listdir(os.path.join(path_data,v,'images')))-self.alternate * self.len_snippet))
